...
int data_imu_pos(){
    ...
    float d,dp,temp,accx,accy,accz,gyrx,gyry,gyrz,velx,vely,velz,latitude,longitude,altitude,roll,pitch,yaw;

// The standard position message structure.

    data_pos[0]=0xFA;data_pos[1]=0xFF;data_pos[2]=0x32;data_pos[3]=0x3D;

    printf("data_pos init:%x \n",data_pos);
...
// make chunks of 100 bytes of data.
				...
				if(j>99) {
					...
					for (i0=0;i0<100-30;i0++){
						...
						printf("modes - data:%d data_pos:%d \n",d,dp);
