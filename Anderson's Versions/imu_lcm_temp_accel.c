...
char tempHolder[6],accxHolder[7],accyHolder[7],acczHolder[7];

char sendBuffer[]="Temp:00.000 AccX:00.0000 AccY:00.0000 AccZ:00.0000 \r\n";  
...
int data_imu_pos(){
    ...
    char data_pos[21],crc,data[50],data_pos1[21];

    int t=0,i,k=0,j=0,l,i1,j1,test,d0,d1,d2,d3,dp0,dp1,dp2,dp3;
...
// The standard position message structure.
    ...
    data_pos[0]=0xFA;data_pos[1]=0xFF;data_pos[2]=0x32;data_pos[3]=0x10;
...
// Get the Lat, Lon, Alt and write them into a Binary Datafile for now.
		...
		if (crc_val==0) {
			...
			printf("Temperature %6.3f AccX:%7.4f AccY:%7.4f AccZ:%7.4f \n",temp,accx,accy,accz);

			sprintf(tempHolder,"%+06.3f",temp);
			...
			for (i=0;i<sizeof(tempHolder);i++) {
				...
				sendBuffer[5+i] = tempHolder[i];

			}

			for (i=0;i<sizeof(accxHolder);i++) {

				sendBuffer[17+i] = accxHolder[i];

			}

			for (i=0;i<sizeof(accyHolder);i++) {

				sendBuffer[30+i] = accyHolder[i];

			}

			for (i=0;i<sizeof(acczHolder);i++) {

				sendBuffer[43+i] = acczHolder[i];
...
int main(){ 
    ...
    msgout[0]=0xFA;msgout[1]=0xFF;msgout[2]=0xD0;msgout[3]=0x02;msgout[4]=00;msgout[5]=0x03;msgout[6]=0x2C;
	...
	msgoutsettings[0]=0xFA;msgoutsettings[1]=0xFF;msgoutsettings[2]=0xD2;msgoutsettings[3]=0x04;msgoutsettings[4]=0x80;msgoutsettings[5]=0x00;msgoutsettings[6]=0x0C;msgoutsettings[7]=0x64;msgoutsettings[8]=0x3B;
