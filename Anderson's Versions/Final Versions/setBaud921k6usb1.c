...
//struct timeval now,start;
    ...
    char msgbaud[6],msgbaudack[5],msgconfig[5],msgconfigack[5],msgout[7],msgoutack[5],msgoutsettings[9],msgoutsettingsack[5],msgmeas[5],msgmeasack[5];
...
int reset_imu(){
    ...
    msgreset[0]=0xFA;msgreset[1]=0xFF;msgreset[2]=0x40;msgreset[3]=0x00;msgreset[4]=0xC1;
...
redo0:
	...
	statwrite=write(fd,&msgbaud,6);
        ...
        if (statwrite!=6) perror("Output settings unsuccessful");
	...
	for (i=0;i<sizeof(buffer)-3;i++) {
            ...
            if (buffer[i]==msgbaudack[0] && buffer[i+1]==msgbaudack[1] && buffer[i+2]==msgbaudack[2] && buffer[i+3]==msgbaudack[3]) {
...
//    else{
    	...
    	if (fd == -1){
			...
			perror("First Attempt Error: Unable to open port /dev/ttyUSB1 - ");

			fd = open("/dev/ttyUSB1", O_RDWR | O_NOCTTY | O_NDELAY);

			if (fd == -1){

				perror("Second Attempt Error: Unable to open /dev/ttyUSB1 - ");
...
int main(){ 
	...
	msgbaud[0]=0xFA;msgbaud[1]=0xFF;msgbaud[2]=0x18;msgbaud[3]=0x01;msgbaud[4]=0x80;msgbaud[5]=0x68;

	//Baud ack

	msgbaudack[0]=0xFA;msgbaudack[1]=0xFF;msgbaudack[2]=0x19;msgbaudack[3]=0x00;msgbaudack[4]=0xE8;
...
// For Config baud
	...
	if(config_imu_baud()!=0)	printf("Error_in_config_imu_baud()\n"); 

		else	printf("Success_in_config_imu_baud\n");
