...
char tranBuffer[]="Lat:0000.000000 Long:0000.000000 Alt:00.00 VelX:00.0000 VelY:00.0000 VelZ:00.0000 AccX:00.0000 AccY:00.0000 AccZ:00.0000 \r\n";  

char rotBuffer[]="Roll:0000.000000 Pitch:0000.000000 Yaw:0000.000000 GyrX:00.0000 GyrY:00.0000 GyrZ:00.0000 \r\n";
...
int data_imu_pos(){
    ...
    char data_pos[69],crc,data[150],data_pos1[69];

    int t=0,i,k=0,j=0,crc_val,l,i0,i1,i2,i3,i4,i5,i6,i7,i8,i9,i10,i11,i12,i13,i14,i15,i16,j1,test,d0,d1,d2,d3,dp0,dp1,dp2,dp3;
...
// Get the Lat, Lon, Alt and write them into a Binary Datafile for now.
		...
		if (crc_val==0) {
			...
			printf("all data in floats \n");


			for (i2=0;i2<sizeof(latHolder);i2++) {

				tranBuffer[4+i2]=latHolder[i2];
				
			}

			for (i3=0;i3<sizeof(longHolder);i3++) {

				tranBuffer[21+i3]=longHolder[i3];
				
			}

			for (i4=0;i4<sizeof(altHolder);i4++) {

				tranBuffer[37+i4]=altHolder[i4];
				
			}

			for (i5=0;i5<sizeof(velxHolder);i5++) {

				tranBuffer[48+i5]=velxHolder[i5];
				
			}

			for (i6=0;i6<sizeof(velyHolder);i6++) {

				tranBuffer[61+i6]=velyHolder[i6];
				
			}

			for (i7=0;i7<sizeof(velzHolder);i7++) {

				tranBuffer[74+i7]=velzHolder[i7];
				
			}

			for (i14=0;i14<sizeof(accxHolder);i14++) {

				tranBuffer[87+i14]=accxHolder[i14];
				
			}

			for (i15=0;i15<sizeof(accyHolder);i15++) {

				tranBuffer[100+i15]=accyHolder[i15];
				
			}

			for (i16=0;i16<sizeof(acczHolder);i16++) {

				tranBuffer[113+i16]=acczHolder[i16];
			...
			for (i8=0;i8<sizeof(rollHolder);i8++) {
				...
				rotBuffer[5+i8]=rollHolder[i8];
				
			}

			for (i9=0;i9<sizeof(pitchHolder);i9++) {

				rotBuffer[23+i9]=pitchHolder[i9];
				
			}

			for (i10=0;i10<sizeof(yawHolder);i10++) {

				rotBuffer[39+i10]=yawHolder[i10];
				
			}

			for (i11=0;i11<sizeof(gyrxHolder);i11++) {

				rotBuffer[56+i11]=gyrxHolder[i11];
				
			}

			for (i12=0;i12<sizeof(gyryHolder);i12++) {

				rotBuffer[69+i12]=gyryHolder[i12];
				
			}

			for (i13=0;i13<sizeof(gyrzHolder);i13++) {

				rotBuffer[82+i13]=gyrzHolder[i13];
				
			}

			

			printf("Buffers updated \n");
...
// For data capture of position data
	...
	printf("Capturing GPS Data\n"); 	
		...
		if(data_imu_pos()!=0)	printf("Error_in_data_imu()\n"); 
			...
			write(fdSG,tranBuffer,sizeof(tranBuffer)); 

			printf("Message Sent.\n");

			printf("Sending: %s\n",rotBuffer); 

			write(fdSG,rotBuffer,sizeof(rotBuffer)); 
