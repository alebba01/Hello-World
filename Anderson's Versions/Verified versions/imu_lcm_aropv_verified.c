...
char accxHolder[7],accyHolder[7],acczHolder[7],gyrxHolder[7],gyryHolder[7],gyrzHolder[7],rollHolder[11],pitchHolder[11],yawHolder[11],latHolder[11],longHolder[11],altHolder[5],velxHolder[7],velyHolder[7],velzHolder[7];

char sendBuffer[]="AccX:00.0000 AccY:00.0000 AccZ:00.0000 GyrX:00.0000 GyrY:00.0000 GyrZ:00.0000 Roll:0000.000000 Pitch:0000.000000 Yaw:0000.000000 Lat:0000.000000 Long:0000.000000 Alt:00.00 VelX:00.0000 VelY:00.0000 VelZ:00.0000 \r\n";  
...
int data_imu_pos(){
    ...
    char data_pos[65],crc,data[80],data_pos1[65];

    int length=65,t=0,i,k=0,j=0,l,i1,j1,test,d0,d1,d2,d3,dp0,dp1,dp2,dp3;
    ...
    float accx,accy,accz,gyrx,gyry,gyrz,roll,pitch,yaw,latitude,longitude,altitude,velx,vely,velz;

// The standard position message structure.

    data_pos[0]=0xFA;data_pos[1]=0xFF;data_pos[2]=0x32;data_pos[3]=0x3C;
...
// make chunks of 50 bytes of data.
				...
				if(j>79) {
					...
					for (i1=0;i1<80-30;i1++){
						...
						printf("Data:%d %d %d %d Data_pos:%d %d %d %d \n",d0,d1,d2,d3,dp0,dp1,dp2,dp3);
...
// Get the Lat, Lon, Alt and write them into a Binary Datafile for now.
		...
		if (crc_val==0) {
			...
			//Bytes 4:15 are accelerations. 4 for each of the X,Y and Z directions in m/s^2, IEEE float format
			...
			//Bytes 16:27 are rate of turns. 4 for each of the X,Y and Z axes in rad/s, IEEE float format
			...
			//Bytes 28:39 are orientation angles. 4 for each of the roll, pitch, and yaw in degrees, IEEE float format
			...
			//Bytes 40:51 are GPS values. 4 for each of the latitude,longitude and altitude. Latitude and longitude are in degrees, altitude is in meters with respect to the LLAWGS84 model of the earth's surface, IEEE float format
			...
			//Bytes 52:63 are velocities. 4 for each of the X,Y and Z directions in m/s, IEEE float format
			...
			printf("AccX:%7.4f AccY:%7.4f AccZ:%7.4f GyrX:%7.4f GyrY:%7.4f GyrZ:%7.4f Roll:%11.6f Pitch:%11.6f Yaw:%11.6f Lat:%11.6f Long:%11.6f Alt:%11.6f VelX:%7.4f VelY:%7.4f VelZ:%7.4f \n",accx,accy,accz,gyrx,gyry,gyrz,roll,pitch,yaw,latitude,longitude,altitude,velx,vely,velz);
...
int main(){ 
    ...
    msgout[0]=0xFA;msgout[1]=0xFF;msgout[2]=0xD0;msgout[3]=0x02;msgout[4]=00;msgout[5]=0x36;msgout[6]=0xF9;
