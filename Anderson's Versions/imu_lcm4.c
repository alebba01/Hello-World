...
//			printf("\n Latitude= %f Longitude= %f Altitude= %f roll= %f pitch= %f yaw= %f %s\n",latitude,longitude,altitude,roll,pitch,yaw,asctime(timeinfo)); 
			...
			if (test>60){ 
				...
				sendBuffer[2]=rollHolder[0];

				sendBuffer[3]=rollHolder[1];

				sendBuffer[4]=rollHolder[2];

				sendBuffer[5]=rollHolder[3];

				sendBuffer[6]=rollHolder[4];

				sendBuffer[7]=rollHolder[5];

				sendBuffer[8]=rollHolder[6];

				sendBuffer[9]=rollHolder[7];

				sendBuffer[10]=rollHolder[8];

				sendBuffer[11]=rollHolder[9];

				sendBuffer[12]=rollHolder[10];



				// Pitch

				sendBuffer[20]=pitchHolder[0];

				sendBuffer[21]=pitchHolder[1];

				sendBuffer[22]=pitchHolder[2];

				sendBuffer[23]=pitchHolder[3];

				sendBuffer[24]=pitchHolder[4];

				sendBuffer[25]=pitchHolder[5];

				sendBuffer[26]=pitchHolder[6];

				sendBuffer[27]=pitchHolder[7];

				sendBuffer[28]=pitchHolder[8];

				sendBuffer[29]=pitchHolder[9];

				sendBuffer[30]=pitchHolder[10];
				...
				sendBuffer[36]=yawHolder[0];

				sendBuffer[37]=yawHolder[1];

				sendBuffer[38]=yawHolder[2];

				sendBuffer[39]=yawHolder[3];

				sendBuffer[40]=yawHolder[4];

				sendBuffer[41]=yawHolder[5];

				sendBuffer[42]=yawHolder[6];

				sendBuffer[43]=yawHolder[7];

				sendBuffer[44]=yawHolder[8];

				sendBuffer[45]=yawHolder[9];

				sendBuffer[46]=yawHolder[10];
