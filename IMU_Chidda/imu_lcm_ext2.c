...
char longHolder[11],latHolder[11];

char sendBuffer[]="GPS Lat:0000.000000 Long:0000.000000 \r\n";  
...
// Get the Lat, Lon, Alt and write them into a Binary Datafile for now.
		...
		if (crc==0) {
			...
			////Bytes 42:53 are GPS values. 4 for each of the X,Y and Z axes in rad/s, IEEE float format
...
redo3:
	...
	perror("Output settings unsuccessful %d", statwrite);

	statwrite=write(fd,&msgoutsettings,9);

        if (statwrite!=9) perror("Output settings unsuccessful %d", statwrite);
